...
def professors(request):
    if request.method == 'GET':
        ...
        return render(request, 'ftn/professors.html', {
        ...
        if registration_form.is_valid():
            ...
            response = requests.post('http://nginx:80/professors', headers={'Content-Type': 'application/json'},
                                     json=body)
            try:
...
def index(request):
    if request.method == 'GET':
        ...
        if registration_form.is_valid():
            ...
            response = requests.post('http://nginx:80/students', headers={'Content-Type': 'application/json'},
                                     json=body)
            try:
                if response.status_code == 200:
                    st.save()
